  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        ...
        let responseData = JSON.parse(response.data.substring(response.data.indexOf('{'))); // Parse the JSON part of the response
        const allRecords =  responseData.data;
        setTotalRecords(prev => prev + allRecords.length);
        setPendingRecords(prev => prev + allRecords.filter(record => record.status === 'Pending').length);
        setApprovedRecords(prev => prev + allRecords.filter(record => record.status === 'Approved').length);
        setRejectedRecords(prev => prev + allRecords.filter(record => record.status === 'Rejected').length);
      } catch (error) {
        console.error("Error fetching records:", error);
      }
    };
